...
> Note: You **can't create** an instance of **`Superhero`** class because it is an abstract class and has an abstract method. This will raise a `TypeError` because the `fly()` method is not implemented.
...
* An interface in Python is just like an abstract class, but with one key difference: ***it contains only abstract methods.***
> While abstract classes can mix both concrete and abstract methods, an interface is a "pure" abstract class. It only defines a contract of what methods must be implemented.
...
* This is an interface because it contains **only abstract methods** `connect` and `query` - ***it serves as a pure contract that specifies what methods implementing classes must provide***.
> While some programming languages have dedicated `interface` keywords, Python achieves the same functionality using abstract classes `(ABC)`.
...
**Example of Interface & Inheritance from multiple classes**
