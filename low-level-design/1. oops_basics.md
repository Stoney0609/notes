...
* Check out [2.2](#22-enumerations) for more info on `Enums`
...
```py
print(f"docstring for the class {a.__doc__}") 
...
## 2. Data Modeling basics in OOPs
* Data Modeling is the process of structuring and representing data in a program so that it reflects real-world entities clearly and safely.
* Good data modeling helps with:
    * Code readability
    * Data validation
    * Preventing invalid states
    * Easier maintenance
    * Better IDE/static analysis support

### 2.1 Constants
* Constants are values that should not change during program execution.
    > Python does not enforce constants strictly, but they are defined by convention.
    
    > Convention - `UPPER_CASE_NAMES`
...
* Instead of `if order.status == "SHIPPED":`, use this -
    ``` py
...
* **Class-Level Constants**
    * Constants can also exist inside classes.
        ``` py
        class Payment:
            MAX_RETRY = 3
            TIMEOUT_SECONDS = 30
        
        if retries > Payment.MAX_RETRY:
            print("Max retries reached")
        ```


### 2.2 Enumerations
* Enums are used to represent a fixed set of valid domain values. An Enum models a closed set of valid states or categories in a domain.
* Enums are mainly used as attributes inside classes.
...
```
When we use like this (below) - we come up with these issues:
...
status = "SHIPPPED"   # typo allowed
```
* No validation
* No type safety
* Invalid states possible
* Domain mixing possible

Enums enforce the following: 

* 2.2.1 **Type Safety**
    * Enum values belong to a specific type. This prevents accidental misuse of raw strings.
        ``` py
        status = OrderStatus.SHIPPED
        type(status)
        ```
...
* 2.2.2 **Restricted Value Set**
    * Only defined values are allowed. This prevents illegal states.
        > This is based on the design principle - **"Illegal states should be impossible to represent"**

        ``` py
        OrderStatus("SHIPPED")     # valid
        OrderStatus("INVALID")     # ValueError
        ```

* 2.2.3 **Domain Isolation**
    * Two enums with the same value remain different. **Enums maintain domain boundaries.**
        ``` py
        class PaymentStatus(Enum):
            SHIPPED = "SHIPPED"

        OrderStatus.SHIPPED == PaymentStatus.SHIPPED
        ```
        > The above code returns `False`
    * make code more expressive, and ensure comparisons occur within the correct semantic domain rather than just comparing raw values.

* 2.2.4 **Iteration Support and metadata**
    * You can iterate over all valid values.
        > useful for validation, dropdown lists etc
        ``` py
        for status in OrderStatus:
            print(status)
        ```
    * Each valid state (member) of the domain has `.name` & `.value`
        ``` py
...
### Constants vs Enums
    > A class that inherits from `Enum` cannot behave like a normal class with instance attributes and typical methods.
* Enums treat every class attribute as an enum value. 
* If a class inherits from `Enum`, that class itself becomes an **enumeration**, not a regular class.
* Therefore, the usual pattern is NOT to mix a domain class and an Enum in the same class.
* **Bad design example** - `MAX_RETRY` is not a **constant**, it becomes an **enum member**, which is wrong. `MAX_RETRY` is not a status — it’s configuration. This breaks the  design principle **Single Responsibility Principle (SRP)**.
    ``` py
    class OrderStatus(Enum):
        PLACED = "PLACED"

        MAX_RETRY = 3   # this becomes another enum member!
    ```
* **Good design example** 
    * Keep Enums and domain classes separate
    ``` py
    from enum import Enum
